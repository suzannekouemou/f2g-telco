...
export async function initCommand(options: InitOptions) {
  ...
  if (installedIds.includes('contextgraph') && env.python) {
    await installContextGraphBridge(os.homedir());
  }
 ...
 */
async function installContextGraphBridge(home: string): Promise<void> {
  const binDir = path.join(home, '.local', 'bin');
  const wrapperPath = path.join(binDir, 'contextgraph-mcp');

  log.info('Setting up ContextGraph bridge...');

  try {
    // Ensure pip installs contextgraph
    execSync('pip install --user contextgraph[server,mcp] --quiet', {
      stdio: 'pipe',
      timeout: 120_000,
    });
...
# ContextGraph MCP bridge wrapper — auto-generated by F2G-Telco
# This wrapper ensures contextgraph-mcp runs with proper Python environment

# Ensure user pip bin is in PATH
export PATH="$HOME/.local/bin:$PATH"

# Execute contextgraph MCP server
exec python3 -m contextgraph.mcp "$@"
`;

    await fs.writeFile(wrapperPath, wrapperScript);
    await fs.chmod(wrapperPath, 0o755);

    log.success(`ContextGraph bridge → ${wrapperPath}`);
  } catch {
    log.warn('ContextGraph bridge setup failed — contextgraph MCP may not work');
  }
}
